...
On 2026-08-26, PR #27 removed pull-request path filters from Agent Hub CI and Phase 5
Deployment Bundle CI. All three required workflows now run for every pull request. The
protection rule was then extended to seven contexts by adding
`validate-n8n-workflows`; the other settings were preserved. A stacked pull request may
remain based on its dependency while under development, but before merge it must be
rebased/retargeted to current `main`; that event is the mandatory full-CI gate.
...
The live verification on 2026-08-26 confirmed `strict=true`, all seven contexts above,
admin enforcement and conversation resolution enabled, with force pushes and branch
deletion disabled.
