...
function render(s){$('mTasks').textContent=s.tasks.length;$('mPending').textContent=s.pending_approvals;$('mExecuted').textContent=s.executed_actions;$('mFailed').textContent=s.failed_actions;$('tasks').innerHTML=s.tasks.map(t=>`<tr><td><code>${esc(t.task_id)}</code></td><td>${esc(t.objective)}</td><td>${(t.selected_agents||[]).map(a=>`<span class="tag">${esc(a)}</span>`).join('')}</td><td>${t.total_actions}</td><td>${t.pending_approvals}</td><td>${t.failed_actions}</td></tr>`).join('');$('audit').innerHTML=(s.recent_audit||[]).map(a=>`<tr><td>${esc(a.created_at)}</td><td>${esc(a.event_type)}</td><td><code>${esc(a.task_id)}</code></td><td>${esc(a.action_id||'')}</td><td>${esc(a.agent||'')}</td></tr>`).join('');loadApprovals(s.tasks)}
...
async function createTask(){const objective=$('objective').value.trim();if(objective.length<3)return setStatus('Mục tiêu quá ngắn','error');try{const r=await api('/api/v1/agent-tasks',{method:'POST',body:JSON.stringify({objective})});$('objective').value='';setStatus('Đã tạo task '+r.task_id,'ok');await refreshAll()}catch(e){setStatus(e.message,'error')}}
if(token)refreshAll();
</script>
</body></html>'''
